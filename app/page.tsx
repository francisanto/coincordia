...
>>>>>>> 83309b13d5a75b38b03a17c3ada38868be08c9b1
  ...
  // Set admin API key
  const ADMIN_API_KEY = "103810103810"

  // Check if current user is admin
  useEffect(() => {
    const isUserAdmin = address?.toLowerCase() === ADMIN_WALLET;
    setIsAdmin(isUserAdmin);
    
    // Auto-grant admin access if wallet matches admin wallet
    if (isUserAdmin) {
      toast({
        title: "✅ Admin Access Granted",
        description: "Admin wallet detected. You now have access to all group data.",
      });
    }
...
>>>>>>> 83309b13d5a75b38b03a17c3ada38868be08c9b1
    ...
    try {
      ...
      toast({
        ...
        description: `"${newGroup.name}" is now live. Your data is securely stored and accessible anytime.`,
        duration: 6000,
      })
...
>>>>>>> 83309b13d5a75b38b03a17c3ada38868be08c9b1
  ...
  const verifyAdminAccess = async () => {
    ...
    try {
      // Use the hardcoded admin API key or grant direct access to the user's wallet
      if (adminApiKey === ADMIN_API_KEY || address?.toLowerCase() === ADMIN_WALLET.toLowerCase()) {
        setIsAdmin(true);
        toast({
          title: "✅ Admin Access Granted",
              ...
              {isLoadingGroups && (
                <div className="text-center py-8">
                  <div className="inline-flex items-center space-x-2 text-white">
                    ...
                    <span>Loading your groups from database...</span>
                  </div>
                </div>
              )}
            </ClientOnly>
                      ...
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                          ...
                          <p className="text-white/70">
                            Enter your admin API key to access all group data stored in the database
                          </p>
                        </div>
