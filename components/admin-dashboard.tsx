...
export function AdminDashboard({
  ...
  const loadAdminData = async (key = apiKey) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Loaded ${data.groups?.length || 0} groups from database`,
      })
  ...
  // Load data if already verified
  useEffect(() => {
    if (isAdmin && adminApiKey && !groups.length) {
      loadAdminData(adminApiKey)
    }
  }, [isAdmin, adminApiKey]) // ✅ no extra brace here
        ...
        <Card className="bg-red-900/20 border-red-500/30">
          ...
          <CardContent>
            <p className="text-red-300/90">
              This area is restricted to authorized administrators only. If you
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {groups.map((group) => (
                        <TableRow
                          key={group.id}
                          className="border-concordia-light-purple/10"
                        >
                ...
                <div className="text-center py-8 text-white/50">
                  {loading ? (
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <p>
                        No data available. Enter your admin API key and click
                        "Load Data".
                      </p>
                    </div>
                  )}
