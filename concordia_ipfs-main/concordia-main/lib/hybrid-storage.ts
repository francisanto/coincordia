...
// All data is stored in IPFS with proper access control
...
class HybridStorageService {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.success && result.groups && result.groups.length > 0) {
          console.log('✅ Successfully loaded user groups from IPFS:', result.groups.length);
      ...
      console.error('❌ Error loading groups from IPFS:', error);
      return [];
    }
  }
  ...
  async getGroup(groupId: string, userAddress?: string): Promise<{ metadata: GroupMetadata; fullData?: any } | null> {
    ...
    try {
      ...
      // User has access, load the group data from IPFS
      ...
      if (response.ok) {
        ...
        if (result.success && result.metadata) {
          console.log('✅ Successfully loaded group from IPFS:', groupId);
      ...
      console.error('❌ Error getting group from IPFS:', error);
      return null;
    }
  }
  ...
  async saveGroup(groupData: any, userAddress: string): Promise<boolean> {
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || '/api';
      console.log('💾 Saving group to IPFS:', groupData.id, 'by user:', userAddress);
      ...
      console.error('❌ Error saving group to IPFS:', error);
      return false;
    }
  }
  ...
  async deleteGroup(groupId: string, userAddress: string): Promise<boolean> {
    try {
      ...
      const result = await response.json();
      if (result.success) {
        console.log('✅ Group deleted successfully from IPFS');
        return true;
      } else if (response.status === 403) {
