...
import { useGroupDeletionHandler } from "@/lib/contract-event-handler"
...
export function GroupDashboard({
  ...
  // Initialize contract event handlers for IPFS cleanup
  useGroupDeletionHandler()
...
function VoteToWithdrawButton({ group, address, setUserGroups }: { group: SavingsGroup, address: string, setUserGroups: React.Dispatch<React.SetStateAction<SavingsGroup[]>> }) {
  const { write: writeContract, isLoading: isPending } = useContractWrite({
    address: CONCORDIA_CONTRACT_ADDRESS,
    abi: CONCORDIA_CONTRACT_ABI,
    functionName: "voteForWithdrawal"
  })
  const { data: groupDetails, refetch } = useContractRead({
    address: CONCORDIA_CONTRACT_ADDRESS,
    abi: CONCORDIA_CONTRACT_ABI,
