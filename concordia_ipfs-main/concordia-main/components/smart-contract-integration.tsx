...
// Define GroupMetadata type here since we removed the import from ipfs-service
...
export function SmartContractIntegration({
  ...
  const { write: writeContract, data: hash, error, isLoading: isPending } = useContractWrite({address: CONCORDIA_CONTRACT_ADDRESS, abi: CONCORDIA_CONTRACT_ABI, functionName: "createGroup"})
  ...
  const handleCreateGroup = async () => {
    ...
    try {
      ...
      // Use MongoDB for storage instead of IPFS
      // Just use the tempObjectId as the metadata hash
      const ipfsHash = tempObjectId
        ...
        try {
          ...
          const updatedMetadata = {
            ...
            updatedAt: new Date().toISOString(),
                ...
                // Update metadata in MongoDB (with timeout)
      try {
        // Import the data persistence service
        const { dataPersistenceService } = await import('@/lib/data-persistence')
        
        const updateResult = await Promise.race([
          dataPersistenceService.updateGroupMetadata(groupId, updatedMetadata),
        ...
        console.warn("Metadata update failed, continuing:", error)
          ...
          // Still call onSuccess with basic data even if MongoDB update fails
        ...
        <CardContent className="p-4">
          ...
          <div className="space-y-2 text-xs text-white/70">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span>Reliable and scalable data storage</span>
            </div>
            <div className="flex items-center space-x-2">
              <ExternalLink className="h-3 w-3" />
              <span>Smart contract linked to database records</span>
            </div>
            <div className="flex items-center space-x-2">
