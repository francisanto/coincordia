...
export default function HomePage() {
  ...
  const handleDisconnect = () => {
    try {
      ...
      if (typeof window !== 'undefined') {
        // Keep these localStorage removals as they are needed for wagmi functionality
  ...
  // Load groups from MongoDB when wallet connects
  useEffect(() => {
    const loadGroups = async () => {
      ...
      try {
        ...
        // Import data persistence service
        const { dataPersistenceService } = await import('@/lib/data-persistence');
        
        // Load groups from MongoDB
        const mongoGroups = await dataPersistenceService.loadGroups();

        // Filter groups where user is creator or member
        const userGroups = mongoGroups.filter((group: any) => {
        ...
        console.log("📊 MongoDB groups found:", userGroups.length);

        // Format and set MongoDB groups
        if (userGroups.length > 0) {
          ...
          console.log("✅ MongoDB groups loaded:", formattedGroups.length);
          
          toast({
            title: "✅ Data Loaded",
            description: "Your groups have been loaded from MongoDB",
            duration: 3000,
          });
        }
      } catch (error) {
        ...
        toast({
          ...
          description: "Failed to load groups from MongoDB. Please try refreshing the page.",
          duration: 5000,
        });
      } finally {
        setIsLoadingGroups(false);
  ...
  // Save groups using MongoDB
  const saveGroup = async (groupData: any) => {
    try {
      console.log("💾 Saving group to MongoDB:", groupData);
      ...
      const { dataPersistenceService } = await import('@/lib/data-persistence');
      const result = await dataPersistenceService.saveGroup(preparedGroupData as SavingsGroup);

      if (result.success) {
        console.log("✅ Group saved to MongoDB successfully");
  ...
  // Load user's groups from MongoDB API when wallet connects
  useEffect(() => {
    const loadUserGroups = async () => {
      ...
      setIsLoadingGroups(true);
      try {
        console.log('📥 Loading user groups from MongoDB API...');
  ...
  const handleDeleteGroup = async (groupId: string) => {
    try {
      ...
      // Use the data persistence service to delete the group from MongoDB
      const { dataPersistenceService } = await import('@/lib/data-persistence');
      const result = await dataPersistenceService.deleteGroup(groupId);
  ...
  const handleGroupCreated = async (newGroup: SavingsGroup) => {
    try {
      ...
      const groupData = {
        ...
        targetAmount: newGroup.targetAmount,
      ...
      if (result.success) {
        ...
        setTimeout(async () => {
          try {
            ...
            const refreshedGroups = await dataPersistenceService.loadGroups();
            
            // Filter for user's groups
            const userGroups = refreshedGroups.filter((group: any) => {
              const isCreator = group.createdBy?.toLowerCase() === address?.toLowerCase() || 
                              group.creator?.toLowerCase() === address?.toLowerCase();
              const isMember = group.members?.some((member: any) => 
                member.address?.toLowerCase() === address?.toLowerCase()
      ...
      toast({
        ...
        description: "Failed to save group to MongoDB",
        duration: 5000,
      });
    }
  }
  ...
  const handleGroupCreatedFromContract = async (groupId: string, txHash: `0x${string}`, contractData: any) => {
    ...
    const newGroup: SavingsGroup = {
      ...
      metadataId: contractData.metadataId, // From contract
        metadataHash: contractData.metadataHash, // From contract
    ...
    try {
      // Save to MongoDB after successful contract creation
