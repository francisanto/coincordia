...
contract Concordia is Ownable, ReentrancyGuard {
    ...
    event GroupMetadataUpdated(uint256 indexed groupId, string newMetadataId, string newMetadataHash);
    event GroupMetadataUpdated(uint256 indexed groupId, string metadataHash);
    event GroupDeleted(uint256 indexed groupId, string ipfsHash, address deletedBy);
    ...
    function createGroup(
        ...
        require(bytes(_metadataId).length > 0, "Metadata ID required");
    ...
    function _executeWithdrawal(uint256 groupId) private {
        ...
        emit GroupDeleted(groupId, group.metadataHash, msg.sender);
    }
    ...
    function updateGroupMetadata(uint256 groupId, string memory newMetadataId, string memory newMetadataHash) 
        external onlyGroupCreator(groupId) groupExists(groupId) {
        require(bytes(newMetadataId).length > 0, "Empty metadata ID");
        require(bytes(newMetadataHash).length > 0, "Empty metadata hash");
        
        groups[groupId].metadataId = newMetadataId;
        groups[groupId].metadataHash = newMetadataHash;

        emit GroupMetadataUpdated(groupId, newMetadataId, newMetadataHash);
    }

    function updateMetadataHash(uint256 groupId, string memory metadataHash) 
        external onlyGroupCreator(groupId) groupExists(groupId) {
        require(bytes(metadataHash).length > 0, "Empty metadata hash");
        
        groups[groupId].metadataHash = metadataHash;
        groupMetadataHashes[groupId] = metadataHash;

        emit GroupMetadataUpdated(groupId, groups[groupId].metadataId, metadataHash);
    }
